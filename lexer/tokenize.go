...
func tokenize(source []byte) ([]token.Token, error) {
	...
	for current < len(code) {
		...
		case '"':
			...
			if current >= len(code) {
				return nil, fmt.Errorf("line %d: unterminated string literal", stringLine)
			}
			advance(&current, &code)
			addToken(code[start:current], token.STRING, stringLine, &tokens)
		...
		default:
			if code[current] >= '0' && code[current] <= '9' {
				...
				return nil, fmt.Errorf("line %d: unexpected character %q", atLine, code[current])
			}
		}
		start = current
	}

	addToken("", token.EOF, atLine, &tokens)
	return tokens, nil
}
